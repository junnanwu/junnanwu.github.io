...
set -e
base_dir='../'
target_catalog="JavaSE JavaEE JavaWeb Framework Database ComputerBasic Dev Tool JavaTool Share Other"
#最终输出
result=""
echo 开始生成目录...
print_dir(){
	...
	then
	    ...
	    final_path=${relative_path}
            file_header=`head -1 ../$final_path`
            file_header=${file_header:2}
            file_prefix=`head -1 ../$final_path| cut -d ' ' -f1`
            if [[ $file_prefix = "#" ]]
            then
                result="$result$space- [$file_header]($final_path)\n"
            else
                #${file%.*}删除最后一个.及其右边内容
