...
- [x] **都有哪些垃圾回收器**
  ...
  上面提到的新生代的serial，parlNew，还有老年代的CMS。Serial和parlNew回收的方式类似，都会暂停所有用户线程，区别是parlNew是多个线程并行进行收集。
...
- [ ] MVCC是什么
  ...
  Mysql为了解决不可重复读，采用了多版本并发控制的方式，一行记录实际上可能有多个版本，每个版本都记录了其事务ID，所以通过这个快照，就能实现可重复读和读已提交。

  读已提交和可重复读的主要的区别就是在快照的创建上，可重复读仅在事务开始时创建一次，而读提交每次执行语句的时候都要重新创建一次。

- [ ] MySQL都会有什么锁

  - 行锁来解决事务并发情况下，将本事务修改的行加行锁
  - 间隙锁会把一个索引树拆分为多个区间，在A事务在修改一条记录的时候，Mysql会该记录附近的区间加上间隙锁，导致其他事务无法执行插入操作，如果操作的不是索引列，则会给整个表加上间隙锁。
